...
if (YII_SITE_TYPE == 'web') {
    $defaultRoute = 'web/vip/index';
} elseif (YII_SITE_TYPE == 'wap') {
    $defaultRoute = 'wap/vip/default';
}
...
$config = [
    ...
    'components' => [
        'request' => [
            ...
            'cookieValidationKey' => 'pGZvet4QgtCTrhqZaqtq0bHJsU44bK82',
            'enableCookieValidation' => false,//true:cookie加密校验
            'csrfParam' => '_csrf-' . $theAppId,
            'enableCsrfValidation' => true,//true:开启csrf校验
        ],
        'user' => [
        ...
        'session' => [
            // this is the name of the session cookie used for login on the markavip
            'name' => 'advanced-' . $theAppId,
        ],
        'log' => [
            'traceLevel' => YII_DEBUG ? 3 : 0,//日志包含N个调用堆栈信息
            'targets' => [
                [
        ...
        'errorHandler' => [
            'errorAction' => $defaultRoute,//site/error
        ],
        'db' => $dbMysqlBrand['mysqlBrand'],
        //'dbTwo' => $dbMysqlBrand['mysqlTwo'],
